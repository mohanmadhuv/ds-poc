...
This catalog defines reusable compositions for common enterprise SaaS tasks. Select a primary pattern before selecting individual components. Use the installed Carbon packages as the runtime authority for exact APIs.
...
- Use Carbon components and tokens before creating custom UI.
...
### Preferred Carbon composition

`UI Shell + Breadcrumb (when hierarchical) + page header + Button + Search + filters + DataTable + Tag + OverflowMenu + Pagination (when warranted) + Modal + Notification`
...
- Hiding the primary create/manage action in an overflow menu.
...
`page header + Search + Select/Dropdown/MultiSelect/DatePicker filters + clear/reset action + structured results (DataTable, list, or detail summary) + result count/status feedback`
...
`page header or Modal + labeled TextInput/TextArea/NumberInput + Select/Dropdown + Checkbox/RadioButtonGroup/Toggle as appropriate + Button + inline validation + Notification`

### Allowed variants

- Use a Modal for a short, focused task with roughly three simple independent fields.
...
`UI Shell + Breadcrumb (when useful) + page header + ProgressIndicator + grouped form inputs + inline validation + Back/Next Buttons + review summary + final Button + Notification`
...
`Breadcrumb (when hierarchical) + page header + status Tag + structured summary + Tabs for peer sections + Accordion for optional detail + contextual Button/OverflowMenu + Notification`
...
- Use a Modal only for compact detail or a focused decision.
...
- Using a Modal for long, multi-section detail.
...
`UI Shell + page header + freshness context + meaningful Tile summaries + BarChart/LineChart when decision-relevant + Search/filters + DataTable + Tag + Tabs or detail view + Notification`

### Allowed variants

- Use Tiles for a small number of actionable summary signals, not decorative KPIs.
...
- Making every operational value a Tile.
...
`UI Shell + Breadcrumb (when useful) + page header + Tabs for peer setting groups + labeled form controls + Toggle/Checkbox + Accordion for optional advanced settings + Button + Notification`
...
- Use Toggle for immediate, reversible changes with clear resulting state.
- Use explicit Save/Cancel actions for grouped changes.
- Use a destructive area with Modal confirmation for irreversible settings.
...
- Using Toggles for changes that require a separate Submit action.
...
`page header + relevant content region + concise empty explanation + Button or Link to the natural next action + optional InlineNotification or supporting helper text`
...
- Use a Tile only when it is the product’s existing empty-state container, not as decoration.
...
`contextual Button or OverflowMenu + Modal + explicit destructive Button + cancel action + Notification after completion`

### Allowed variants

- Use a destructive Button directly for low-risk reversible actions when Carbon guidance and the product context support it.
- Use a Modal for irreversible or difficult-to-reverse actions.
- Use an OverflowMenu for low-frequency row actions, but keep consequence communication in the confirmation.
...
- Confirmation Modal with Cancel and explicit destructive action labels.
...
Do not use a confirmation Modal for harmless, reversible, or easily undoable actions when the interruption would slow routine work.
...
`local Loading or Skeleton + InlineNotification/Notification + scoped retry or next-action Button + preserved content context`

### Allowed variants

- Use Skeleton when the content structure is known and retrieval is initial or non-instant.
- Use InlineLoading for localized processing such as a row or form action.
- Use InlineNotification for persistent, region-specific feedback.
- Use toast-style Notification for short-lived global confirmation where context is obvious.
...
- Showing success toasts for every trivial interaction.
